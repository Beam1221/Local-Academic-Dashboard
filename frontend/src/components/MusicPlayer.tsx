...
const genres = ['jazz', 'ambient', 'classical', 'lofi', 'chillout'];
...
export function MusicPlayer({active,onOpen}:{active:boolean;onOpen:()=>void}) {
  ...
  const [source,setSource] = useState<'local'|'radio'|'liked'>('local');
  ...
  async function toggle(){if(!audio.current||!current)return;if(!audio.current.paused)audio.current.pause();else try{await audio.current.play()}catch{setError('Unable to play this source. Try another track.')}}
  function like(t:Track){setFavorites(prev=>prev.some(x=>x.id===t.id)?prev.filter(x=>x.id!==t.id):[...prev,t])}
  async function searchRadio(nextGenre=genre) {
    const token=++searchId.current;setSearching(true);setError('');setSource('radio');
    try{const rows=await request<Station[]>(`/music/radio/search?q=${encodeURIComponent(query)}&genre=${encodeURIComponent(nextGenre)}`);if(token===searchId.current)setStations(rows.map(t=>({...t,id:`radio:${t.id}`,source:'radio',subtitle:[t.country,t.tags.split(',').slice(0,2).join(' · ')].filter(Boolean).join(' · ')})));}
  ...
  return <>
    ...
    <section className="music-library" hidden={!active}>
      <div className="music-hero"><div><span className="music-kicker"><Headphones size={16}/>SOUNDTRACK YOUR STUDY</span><h2>Find your focus.</h2><p>Your songs. A little jazz. Space to concentrate.</p><div className="music-hero-actions"><label className="button primary"><Upload size={17}/>{busy?'Uploading…':'Upload music'}<input aria-label="Upload music files" type="file" multiple accept=".mp3,.wav,.ogg,.flac,.m4a,.webm" disabled={busy} onChange={e=>{void upload(Array.from(e.target.files||[]));e.target.value=''}} hidden/></label><button className="button" onClick={()=>void searchRadio()}>Discover radio</button></div></div><div className="music-art" aria-hidden="true"><div className="vinyl"><div/></div></div></div>
